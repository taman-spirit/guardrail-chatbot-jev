...
8. 某轮被扣留后，向模型发送 `responder.model_history(session, lang)`（Go：`responder.ModelHistory`），而不是原始历史。被扣留的轮次替换为注明违规类别的说明（绝不包含原文），以及用户实际收到的回复。这样当用户说“照做吧”或“我的第一个请求”时，模型知道自己拒绝了什么，而不会去猜测或说看不到那条消息。违规类别保存在 `session.as_state()` 中，因此会话在请求之间存入存储、再用 `Session.from_state()` 恢复后依然有效。
