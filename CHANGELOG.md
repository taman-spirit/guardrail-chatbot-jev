...
## [Go 1.3.3, Viet Nam compliance 1.2.3] - 2026-09-26

### Changed

- Includes Go 1.2.4: `Session.ModelHistory()` tells the chat model which categories withheld a turn
  instead of dropping it, and the reason is kept in `AsState` / `SessionFromState`.
- `Responder.ModelHistory(session, lang)` now works on a session restored from a store: the group,
  the reply and the crisis line come back the same as before it was stored, and the same in Go and
  Python.
...
- A request to hurt others is no longer answered as self-harm. The crisis route is taken only when
  ...
  The verdict is unchanged: the content is still held.
- After a withheld turn the chat model no longer guesses. `Responder.ModelHistory(session, lang)`
...
- `withheld`: the note and per-group labels `Responder.ModelHistory` uses.

## [Go 1.3.1, Viet Nam compliance 1.2.1] - 2026-09-26
...
- README, Definitions: a table of the settings in use and of every category's thresholds, including
  ...
  words before the formulas; each formula block followed by links to the code, which now land on
  the right lines of this branch. In English, Vietnamese, French and Japanese.
